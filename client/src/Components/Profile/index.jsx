      ...
      <Container className={classes.container} maxWidth="sm">
        <Paper className={classes.paper} elevation={3}>
          {profile && profile.user && (
            <FlexCenterBox>
              {/* <Avatar>{profile.user.name.slice(-1)}</Avatar> */}
              <Typography variant="button" style={{ fontSize: "2rem" }}>
                {profile.user.name}
              </Typography>
              ...
              profile.social.map((item) => (
                <div key={item.name + item.href.toString().slice(0, 5)}>
                  <ListItem>
                    ...
                    <ListItemText
                      primary={
                        <Typography variant="body1">{item.href}</Typography>
                      }
                      secondary={item.name}
                    />
                    <ListItemSecondaryAction>
                      ...
                      />
                      <IconButton
                        onClick={() => handleOpenLink(item.name, item.href)}
                        edge="end"
                        aria-label="delete"
                      >
